...
function MedicareDataSource() {
  ...
  var that = this;
  $.get('printzone.csv', function(data) {
...
MedicareDataSource.prototype.FEATURES_ = new storeLocator.FeatureSet(
  //new storeLocator.Feature('Wheelchair-YES', '휠체어 사용 가능'),
  //new storeLocator.Feature('Audio-YES', '오디오 사용 가능')
);
...
MedicareDataSource.prototype.parse_ = function(csv) {
  ...
  for (var i = 1, row; row = rows[i]; i++) {
    ...
    var titles = this.join_([row.Fcilty_nam, row.Shp_num_an], ', ');
    ...
    var store = new storeLocator.Store(row.uuid, position, features, {
      ...
      img: row.Fcilty_typ_2,
      university: row.Locality
    });
    stores.push(store);
  }
  return stores;
